import { ApplicationCommandData, ApplicationCommandOptionData, Collection, CommandInteraction, ContextMenuInteraction,/* Interaction,*/ Snowflake } from 'discord.js';
import fs from 'fs';
...
export async function Load(client: IBot): Promise<void> {
    Bot = client
    ...
    try {
        ...
        for (const file of menuFiles) {
            const command: ICommand = await import(PathCM + file);
...
export async function Deploy(): Promise<void> {
    ...
    ContextMenus.forEach(C => {
        CommandData.push({ name: C.name, type: 'USER' })
    });
